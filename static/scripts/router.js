...
import { Button, render_btn } from './button.js';
import { store } from './store.js';
import { start_update } from "./api.js";
class Block {
    constructor(obj) {
    ...
    start() {
        window.onpopstate = event => {
            if (localStorage.getItem('login') && (event.currentTarget.location.pathname.indexOf('main') || !event.currentTarget.location.pathname.indexOf('registration'))) {
                this.go('/static/route/chat.html');
            }
            else {
...
if (localStorage.getItem('login')) {
    ...
    start_update('sign', object)
        .then(data => {
        router
            .use("/static/route/main.html", Block, store.sign)
            .use("/static/route/registration.html", Block, store.registration)
            .use("/static/route/profile.html", Block, store.profile)
            .use("/static/route/profile_edit.html", Block, store.profile_edit)
            .use("/static/route/pass_edit.html", Block, store.pass_edit)
            .use("/static/route/chat.html", Block, store.chat)
            .use("/static/route/page_404.html", Block, store.page_404)
            .use("/static/route/page_500.html", Block, store.page_500)
            .start();
    }).catch(e => {
        alert(e);
    });
